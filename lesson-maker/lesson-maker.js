...
var showSyllabus = function(syllabus_id) {
    ...
    $.ajax({
	   ...
	   function showLesson(lessonID) {
		...
		if (syllabus.lessons[lessonID].slides) {	
		    $("#inputSlides").val(syllabus.lessons[lessonID].slides);	
		} else {
			$("#inputSlides").val("");
		}
		if (syllabus.lessons[lessonID].wrapup) {	
		    $("#inputWrapup").val(syllabus.lessons[lessonID].wrapup);	
		} else {
			$("#inputWrapup").val("");
		}
		$("#readingList").html("");
	...
	function setSaveBtn() {
		 $("#saveLesson").click(function() {
			 ...
			 syllabus.lessons[$(this).parent().parent().attr("data-id")].slides = $('#inputSlides').val();
			 syllabus.lessons[$(this).parent().parent().attr("data-id")].wrapup = $('#inputWrapup').val();
			$.ajax({
                      type: "POST",
                      url: "update-lesson.php",
                      data: {topics: $("#lesson-topic").text(), lesson:$(this).parent().parent().attr("data-id") , description: $("#lesson-desc").text() , blog: $('#inputBlog').html() , slides:  $('#inputSlides').val() ,  wrapup:  $('#inputWrapup').val()  },
                      datatype: "JSON"
                      }).done(function( data) {
