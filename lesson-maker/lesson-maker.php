 ...
 { 
  while ($syllabus = $syllabi->fetch_object())
  {
   ...
   /* Select the lessons in date order */
	if ($lessons = $mysqli->query("SELECT lesson_id, topics, description,  lesson.lesson_date as sort_date, DATE_FORMAT(lesson_date,'%M %e, %Y' ) as lesson_date ,blogpost  , slides , wrapup FROM lesson  WHERE syllabus_syllabus_id = ".$syllabus->syllabus_id." ORDER BY sort_date ASC")) {
