			   ...
			   if (value.definition) {
	               $.each(value.definition,function(index2,value2) {
					   if (value2.email === user) {
			         		content += '<dd>('+value2.class+') '+value2.definition+'&mdash;<small>'+value2.first_name+'</small>&nbsp;<a class="btn btn-mini remove-definition" data-term="'+ value2.term_term +'" data-id="'+value2.definition_id+'"><i class="icon-remove"></i></a></dd>';
					   } else {
						   content += '<dd>('+value2.class+') '+value2.definition+'&mdash;<small>'+value2.first_name+'</small></dd>';
					   }
			       });
			   };
		   });
